...
function cmdTransport(destFile) {
    ...
    var stream = through.obj(function(file, enc, cb) {
        ...
        var search = /\bdefine\(/;
        if (file.isNull()) {
            this.emit("error", new gutil.PluginError(PLUGIN_NAME, "File is null"));
            return cb();
        }
        if (file.isBuffer()) {
        ...
        var concatenatedFile = new gutil.File({
            ...
            contents: new Buffer(JSON.stringify(combo))
        });
        this.push(concatenatedFile);
        return;
        cb();
    });
    function setDependencies(combos, moduleId) {
        var depend = AllDeps[moduleId];
        if (depend)
            depend.forEach(function(moduleId) {
                !~combos.indexOf(moduleId) && combos.push(moduleId);
                setDependencies(combos, moduleId)
            })
    }
...
function parseNormalId(baseId, deps) {
    if (!baseId) return;
    var ret = [], pathDep, baseSem = baseId.replace(/\"/g, "").split("/"), single = [], baseSemLen = baseSem.length || 0, pathDepLen;
